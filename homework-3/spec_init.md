...
- **IN-1 — Money**: Represent money as `{ amount: Decimal, currency: ISO-4217 }`. Reject mixed-currency comparisons; a cap and the transaction it gates must share currency or be converted upstream (out of scope here — assume same currency).
...
### Beginning context
- `card_id` and `user_id` exist in an upstream account service (assumed; accessed via interface, not owned here).
- An authorization/event bus exists onto which this service publishes `cap.changed` and `cap.denied` events (assumed).
- Empty `spending-cap` service module: no models, no persistence, no API yet.
- A shared logging utility exists but has **no** masking helper yet.

### Ending context
- `models/cap.py` — Cap, CapPeriod, CapScope, FraudHold domain models.
- `models/spend_window.py` — accumulated spend per cap window.
- `services/cap_service.py` — CRUD + precedence resolution.
- `services/enforcement.py` — `evaluate(transaction)` decision engine.
- `services/audit.py` — append-only audit writer.
- `utils/masking.py` — single masking helper for identifiers/money.
- `api/routes.py` — REST endpoints for cap CRUD, evaluate, audit read.
- `tests/` — unit, integration, and edge-case suites with fixtures.
- Compliance note in `docs/audit-fields.md` listing every audited field.
...
**Review checkpoints**: (a) compliance review of `docs/audit-fields.md` against NFR-AUD-2; (b) security review confirming no PAN/PII path into logs; (c) manual masking spot-check of every support-facing response shape.

**Test categories required**: unit (models, precedence, masking, windows), integration (CRUD + audit transaction atomicity), e2e/decision (evaluate matrix), concurrency (EC-3), failure injection (EC-8).
...
**Prompt**: "Create immutable domain models for Cap, CapPeriod (enum daily/weekly/monthly/per_transaction), CapScope (account/card + optional category), and FraudHold, using a Decimal money value object with currency."
**File**: `models/cap.py`, `models/money.py`
**Function/class**: `Cap`, `CapPeriod`, `CapScope`, `FraudHold`, `Money`
**Details**: Money is Decimal + ISO-4217 currency; reject non-positive amounts in `Cap`. UUIDv4 IDs.
**Acceptance**: Models reject amount ≤ 0 and unknown period; Money rejects float input and mixed-currency comparison; 100% of model branches unit-tested.
...
**File**: `models/spend_window.py`, `services/accumulator.py`
**Function/class**: `SpendWindow`, `Accumulator.record(transaction)`
...
**File**: `services/cap_service.py`
**Function/class**: `CapService.{create,get,list,update,delete}`
...
**File**: `services/precedence.py`
**Function/class**: `resolve_binding_caps(card_id, context)`
...
**File**: `services/enforcement.py`
**Function/class**: `Enforcement.evaluate(transaction) -> Decision`
...
**File**: `services/audit.py`, `docs/audit-fields.md`
**Function/class**: `AuditLog.record(event)`
...
**Function/class**: `mask_card`, `mask_email`
**Details**: IN-8 — the only allowed path to render an identifier to a human; card → `•••• 4321`, email → `r***@gmail.com`.
...
**File**: `services/fraud_hold.py`
**Function/class**: `FraudHold.place`, `FraudHold.release`
...
### 9. REST API routes (MO-1, MO-3, MO-6)
**Prompt**: "Expose REST endpoints for cap CRUD, evaluate, fraud hold, and audit read with role-scoped authorization and the error-code mapping."
**File**: `api/routes.py`
**Function/class**: route handlers
**Details**: IN-6 error mapping; pagination NFR-PERF-3; audit read restricted to fraud/compliance; DENY is `200` not an error.
**Acceptance**: Endpoint tests assert status codes per IN-6; pagination caps at 100; audit endpoint `403` for users/support.
...
**File**: `tests/`
**Function/class**: test modules + `conftest` fixtures
**Details**: cover all edge cases and the verification matrix; include a degraded-mode and DST-rollover fixture.
**Acceptance**: Every EC-row has a named test; CI runs all categories; coverage ≥ 90% on `services/` and `models/`.
...
**File**: `tests/perf/test_evaluate_perf.py`
**Function/class**: `test_evaluate_latency`, `test_evaluate_throughput`
**Details**: NFR-PERF-1/5; report p95/p99; run against a representative cap fixture set.
